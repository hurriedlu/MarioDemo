...
public class BackGround {
    ...
    public BackGround(int sort, boolean flag) {
        ...
        if (flag) {//如果这个flag标志是true的话，说明是最后一个场景
        ...
        if (sort == 1) {
            ...
            for (int i = 0; i < 27; i++) {//窗口总共800像素大，图片一个30像素宽，800/30 上取整后为 27
                obstacleList.add(new Obstacle(i * 30, 420, 1, this));//类型1表示是上地面
            }

            //绘制下地面
            for (int j = 0; j <= 120; j += 30) {//因为一个图片是30x30，因此必须过30才到下一个图片的位置
                for (int i = 0; i < 27; i++) {
                    obstacleList.add(new Obstacle(i * 30, 570 - j, 2, this));//类型2，表示下地面
                }
            }

            //绘制砖块A
            for (int i = 120; i <= 150; i += 30) {
                obstacleList.add(new Obstacle(i, 300, 7, this));//横坐标 i ，纵坐标300，类型7表示是蓝色的砖块
            }
            ...
            for (int i = 300; i <= 570; i += 30) {
                if (i == 360 || i == 390 || i == 480 || i == 510 || i == 540) {
                    ...
                    obstacleList.add(new Obstacle(i, 300, 0, this));//0：普通砖块
                }
            }
            ...
            for (int i = 420; i <= 450; i += 30) {
                obstacleList.add(new Obstacle(i, 240, 7, this));//最上面的两个蓝色砖块
            }
            ...
            for (int i = 360; i <= 600; i += 25) {
                if (i == 360) {
                    obstacleList.add(new Obstacle(620, i, 3, this));//左水管
                    obstacleList.add(new Obstacle(645, i, 4, this));//右水管
                }
                else {
        ...
        if (sort == 2) {
            ...
            for (int i = 270; i <= 330; i += 30) {
                if (i == 270 || i == 330) {
                    obstacleList.add(new Obstacle(i, 360, 0, this));//普通砖块
                }
                else {
                    obstacleList.add(new Obstacle(i, 360, 7, this));//不可破坏的蓝色砖块
                }
            }
